...
contract TheLastDegenStanding is ERC721 {
    ...
    /// STEP 1 - JOIN THE GAME
    /**
    * @dev before joining you should approve this contract to transfer 500k $degen.
    * to approve you need to (1) go to https://basescan.org/address/0x4ed4E862860beD51a9570b96d89aF5E1B0Efefed#writeContract#F1
    * (2) connect your wallet
    * (3) in the address field input this contract address (0x7255791F9B2D44863C21Ed1f484d444A74731aC6)
    * (4) in the value field input 500000000000000000000000 (this is 500k + 18 zeros to account for token decimals)
    */
    function join() public {
        ...
        $PLAYER_NFT.playedGame(msg.sender);
    ...
    /**
    * @dev execute this function if no degens have joined the game in the past 24 hours
    */
    ...
    /**
    * @dev call this function often enough, or you will lose :)
    */
    ...
    /**
    * @dev use this function to know how many hours left you have to say GM
    */
    function myNextGmInHours(address player) public view returns (uint256) {
        uint256 diff = (getLastSeen(player) + getGmFrequency()) - block.timestamp;
        return diff / 3600;
    }
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        getDescription(),
                        '", "image": "',
                        $TLDS_METADATA.getGameImageURI(address(this), tokenId),
                        '"}'
                    )
                )
            )
